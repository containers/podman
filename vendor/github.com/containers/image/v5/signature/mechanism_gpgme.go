...
func (m *gpgmeSigningMechanism) keyIdentityForVerificationKeyIdentity(keyIdentity string) (string, error) {
	...
	if subkey == nil {
		...
		// In theory gpgme could fail the if (gpg --list-keys --with-colons) misses a "pub:" line;
		// but gpg (in recent enough versions) prints that unconditionally.
		return "", fmt.Errorf("key %q has no subkeys", keyIdentity)
	}
	// Similarly, subkey.Fingerprint() could be nil if the output is missing an unconditionally-included "fpr:" line. // codespell:ignore fpr
	return subkey.Fingerprint(), nil
}

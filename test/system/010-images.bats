...
EOF
    ...
    # CONTAINER_DEFAULT_PLATFORM should take precedence over containers.conf.
...
EOF
    ...
    CONTAINERS_CONF_OVERRIDE=$containersconf CONTAINER_DEFAULT_PLATFORM="linux/${host_arch}" \
        run_podman pull -q --policy=always $IMAGE
    CONTAINERS_CONF_OVERRIDE=$containersconf CONTAINER_DEFAULT_PLATFORM="linux/${host_arch}" \
        run_podman inspect --format '{{.Architecture}}' $IMAGE
    is "$output" "$host_arch" "CONTAINER_DEFAULT_PLATFORM overrides containers.conf"

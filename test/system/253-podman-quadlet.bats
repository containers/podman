...
EOF
    ...
    assert "$line_count" -eq 1 "install output should contain exactly one line"
    ...
    assert "$output" =~ "Not loaded" "loaded status should be set"
    ...
    # Count lines in output (should be 1 line with the quadlet name)
    local line_count=$(echo "$output" | wc -l)
    assert "$line_count" -eq 1 "remove output should contain exactly one line"
...
@test "quadlet verb - install with external file mount" {
    ...
    random_part=$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c8)
    mount_content="This is a test file for quadlet mount testing $random_part"
...
@test "quadlet verb - install, start service, and force remove running quadlet" {
    # Create a simple quadlet file that runs a long-lived container
    local quadlet_file=$PODMAN_TMPDIR/lifecycle-test.container
    cat > $quadlet_file <<EOF
[Container]
Image=$IMAGE
...
EOF
    ...
    # Get the expected service name for the quadlet
    local service_name=$(quadlet_to_service_name "lifecycle-test.container")

    # Install the quadlet
    run_podman quadlet install $quadlet_file
    assert $status -eq 0 "quadlet install should succeed"

    # Verify the quadlet was installed
    run_podman quadlet list
    assert "$output" =~ "lifecycle-test.container" "list should contain lifecycle-test.container"

    # Verify systemd unit exists and is loaded
    run systemctl status "$service_name"
    assert $status -eq 3 "systemd unit should exist (inactive/loaded state)"
    assert "$output" =~ "Loaded: loaded" "systemd unit should be loaded"

    # Start the systemd service using proper test helper
    systemctl daemon-reload
    run systemctl_start "$service_name"
    assert $status -eq 0 "systemd should be able to start the service"

    # Give the service a moment to start up properly
    sleep 2

    # Verify service is active and running
    run systemctl is-active "$service_name"
    assert $status -eq 0 "service should be active"
    assert "$output" = "active" "service should report as active"

    # Verify service is running (should show as active/running)
    run systemctl status "$service_name"
    assert $status -eq 0 "service status should be successful for running service"
    assert "$output" =~ "Active: active \\(running\\)" "service should be active and running"

    # Try to remove without --force (should fail because service is running)
    run_podman 125 quadlet rm lifecycle-test.container
    assert "$output" =~ "is running and force is not set" "should fail to remove running quadlet without --force"

    # Verify quadlet is still there and service is still running
    run_podman quadlet list
    assert "$output" =~ "lifecycle-test.container" "quadlet should still be listed after failed removal"

    run systemctl is-active "$service_name"
    assert $status -eq 0 "service should still be active after failed removal"
    assert "$output" = "active" "service should still report as active"

    # Now remove with --force (should succeed and stop the running service)
    run_podman quadlet rm --force lifecycle-test.container
    assert $status -eq 0 "quadlet remove with --force should succeed"

    # Verify the quadlet was removed from podman
    run_podman quadlet list
    assert "$output" !~ "lifecycle-test.container" "list should not contain lifecycle-test.container after forced removal"

    # Verify systemd unit no longer exists
    run systemctl status "$service_name"
    assert $status -eq 4 "systemd unit should not exist after removal"
}

...
// skipped. This should not be interpretated as the build tests having failed.
//
...
// of the native architecure be modelled. Hopefully this will be rectified shortly.
//
...
func makeReverseProxy(remoteSocket string) (*ReverseProxy, error) {
	...
	go func() {
		// Runs in hte background until the https server is
		// shutdown
		defer GinkgoRecover()
...
var _ = Context("Testing farm build functionality :", Ordered,  func() {
	...
	//  and is NOT modified by the tests in any way. Consequenly, it is ok to
	...
	var prepareContextsDir = func(baseTmpDir string, testSrc testImageDescriptor) string {
		//
		//Creates:
		//  * if necessary, a root direcory capable of holding a series of sub-direcories.
		//
	...
	var setupConnectionConfigs = func(baseDir string) (string, string) {
		...
		return containersFile, connectionsFile
	}
	/*##########################################################################################################*/
	...
	var setupStaticTest = func(inpDir string) (podmanTest *PodmanTestIntegration, err error) {
		...
		// does a check for Rootfulness, and somtimes it gets the answer wrong. I think if the user
		// is running in a group with a degree of elevated privilege, then it decides it
		// IS rootful. However it then tries to create a socket in a privilged area, only
	...
	// PodmanLocal
	//
	// PodmanLocal is essentially just the podmanTest strucure that would have been
	...
	var NewPodmanLocal = func(baseDir string) *PodmanLocal {
		...
		return &PodmanLocal{
			...
			cleanup: func() {
				...
				// Note: it creates its files under the working direcory of
				// pi, so will be automatically cleaned up when pi is cleaned up.
				var tmp_remote_podmanTest *PodmanTestIntegration = PodmanTestCreateUtil(podmanTempDir, true)
	...
	BeforeAll(func() {
		...
		//
		// We do not do ANY farm build testing in executables bult using the localintegratinon
		// target.
		...
		// use NOT the podmanCmd associated with its That is because, for a 'remote' instatiation
		...
		// traffic to the podman.socket. Note that there is no seperate server process.
		...
		// Write out our Container files with instuctions to build on our standard
		// test image
		contextsDir = prepareContextsDir(podmanStaticTest.TempDir, standardTestImage)

		containersConf, connectionsConf = setupConnectionConfigs(podmanStaticTest.TempDir)
	...
	Context("Preparing a static test env.   :", Ordered, func() {
		...
		// to initialising contants ONLY, and only becuase you want the value to be accessed in areas
		// where they are NOT in scope of one of the Setup Nodes e.g. table entries.

		BeforeEach(func() {
			// This gives each test visiblity to the isolated
		...
		DescribeTable("Creating Connections:",
			func(name string, url string, identity string) {
				...
				// reporting the correct native architecure
				cmd = exec.Command(podmanStaticTest.PodmanBinary,
			...
			// If you use variables in the Entry statments below, the value they will assume will the value they had
		...
		DescribeTable("Adding Connections  :",
			...
			// NOTE: If you use variables in the Entry statments below, the value they will assume
	...
	Describe("Performing farm build tests  :", func() {
		...
		var indicateJSONthatConformsToSpecification = func(bldSpecs []build) types.GomegaMatcher {
			...
			// 	* the architecure of the different images present is as expected.
		...
		var verifyAnyRetainedImages = func( podmanTestInteg *PodmanTestIntegration, bldSession *PodmanSessionIntegration, expectedBuilds expectBuildsOf ) ([]*PodmanSessionIntegration, error){
			...
			var imgSessions []*PodmanSessionIntegration = []*PodmanSessionIntegration{}
		...
		BeforeEach(func() {
			...
			// The standard podmanTest strucure automatically created by the
		...
		DescribeTableSubtree("Basic builds        :",
			...
			// environment only provides that funtionality when the ginkgo test executable is built using the
		...
		DescribeTableSubtree("Failure Scenarios        :",
			...
			//
			Entry("proxyFarm build with --platforms= HOST_ARCH + anyOther",

...
var _ = Describe("Podman pod start", func() {
	...
	It("podman pod start multiple pods", func() {
		...
		session = podmanTest.Podman([]string{"create", "--pod", "foobar99", nginx})
		session.WaitWithDefaultTimeout()
		Expect(session.ExitCode()).To(Equal(0))
		...
		session = podmanTest.Podman([]string{"create", "--pod", "foobar100", nginx})
		session.WaitWithDefaultTimeout()
		Expect(session.ExitCode()).To(Equal(0))
	...
	It("podman pod start multiple containers with bogus", func() {
		session := podmanTest.Podman([]string{"pod", "create", "--name", "foobar99"})
		session.WaitWithDefaultTimeout()
		cid1 := session.OutputToString()

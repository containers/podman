...
var _ = Describe("Podman generate systemd", func() {
	...
	It("podman generate systemd with --header", func() {
		session := podmanTest.Podman([]string{"create", "--name", "foobar", "alpine", "top"})
		session.WaitWithDefaultTimeout()
		Expect(session).Should(Exit(0))

		session = podmanTest.Podman([]string{"generate", "systemd", "foobar", "--header=\"# Foo\""})
		session.WaitWithDefaultTimeout()
		Expect(session).Should(Exit(0))
	...
	It("podman generate systemd with --header --no-header", func() {
		session := podmanTest.Podman([]string{"create", "--name", "foobar", "alpine", "top"})
		session.WaitWithDefaultTimeout()
		Expect(session).Should(Exit(0))

		session = podmanTest.Podman([]string{"generate", "systemd", "foobar", "--header=\"# Foo\"", "--no-header"})
		session.WaitWithDefaultTimeout()
		Expect(session).Should(Exit(0))
	...
	It("podman generate systemd pod with --header", func() {
		session := podmanTest.Podman([]string{"pod", "create", "--name", "foobar"})
		session.WaitWithDefaultTimeout()
		Expect(session).Should(Exit(0))

		session = podmanTest.Podman([]string{"generate", "systemd", "foobar", "--header=\"# Bar\""})
		session.WaitWithDefaultTimeout()
		Expect(session).Should(Exit(0))
	...
	It("podman generate systemd pod with --header --no-header", func() {
		session := podmanTest.Podman([]string{"pod", "create", "--name", "foobar"})
		session.WaitWithDefaultTimeout()
		Expect(session).Should(Exit(0))

		session = podmanTest.Podman([]string{"generate", "systemd", "foobar", "--header=\"# Bar\"", "--no-header"})
		session.WaitWithDefaultTimeout()
		Expect(session).Should(Exit(0))

...
func pullChunkedTests() { // included in pull_test.go
	...
	for _, c := range []struct {
		...
		// Test fresh pull
		c.fresh.testPull(c.img, c.insecureStorage)
		...
		c.reuse.testPull(c.img, c.insecureStorage)
...
func (expectation *pullChunkedExpectation) testPull(image *pullChunkedTestImage, insecureStorage bool) {
	session := podmanTest.Podman([]string{"--log-level=debug", "--pull-option=enable_partial_images=true", fmt.Sprintf("--pull-option=insecure_allow_unpredictable_image_contents=%v", insecureStorage),

    ...
    def test_pull(self):
        ...
        for stream_case in stream_cases:
            with self.subTest(stream_case=stream_case):
                r = requests.post(self.uri(f"/images/pull?reference={existing_reference}{stream_case['quiet_postfix']}"), timeout=15)
                ...
                r = requests.post(self.uri(f"/images/pull?reference={non_existing_reference}{stream_case['quiet_postfix']}"))

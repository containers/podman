...
func checkRuntimeConfig(db *bolt.DB, rt *Runtime) error {
	...
	checks := []dbConfigValidation{
		...
		},
		{
			...
			rt.config.Libpod.StaticDir,
			staticDirKey,
			"",
		},
		{
			"libpod temporary files directory (tmpdir)",
			rt.config.Libpod.TmpDir,
			tmpDirKey,
			"",
		},
		{
			"storage temporary directory (runroot)",
			rt.config.Libpod.StorageConfig.RunRoot,
			runRootKey,
			storeOpts.RunRoot,
		},
		{
			"storage graph root directory (graphroot)",
			rt.config.Libpod.StorageConfig.GraphRoot,
			graphRootKey,
			storeOpts.GraphRoot,
		},
		{
			"storage graph driver",
			rt.config.Libpod.StorageConfig.GraphDriverName,
			graphDriverKey,
			storeOpts.GraphDriverName,
		},
		{
			"volume path",
			rt.config.Libpod.VolumePath,
			volPathKey,
			"",
		},
	}

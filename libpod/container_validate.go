...
func (c *Container) validate() error {
	...
	if len(c.config.ArtifactVolumes) > 0 {
		...
		for _, artifactMount := range c.config.ArtifactVolumes {
			...
			_, err = artStore.BlobMountPaths(context.Background(), asr, &libartTypes.BlobMountPathOptions{
			...
			if err != nil {
				return fmt.Errorf("artifact %s not found in store: %w", artifactMount.Source, err)
			}
		}
	}

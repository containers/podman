...
func (r *Runtime) Migrate(newRuntime string, migrateDB bool) error {
	...
	if migrateDB {
		...
		if printContainersConfWarning {
			logrus.Errorf("In containers.conf, database_backend is manually set to \"boltdb\" - comment this line out to complete migration")
		}
	}

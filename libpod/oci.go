...
func newOCIRuntime(name string, paths []string, conmonPath string, runtimeFlags []string, runtimeCfg *config.Config) (OCIRuntime, error) {
	// Check if runtime supports the Shimv2 implementation
	if stringInSlice(name, runtimeCfg.Engine.RuntimeSupportsShimv2) {
		return NewShimv2Runtime(name, paths, runtimeFlags, runtimeCfg)
	}

	// This is the default OCI runtime

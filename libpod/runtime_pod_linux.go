...
func (r *Runtime) NewPod(ctx context.Context, options ...PodCreateOption) (_ *Pod, deferredErr error) {
	...
	if r.config.Libpod.Namespace != "" {
		pod.config.Namespace = r.config.Libpod.Namespace
	}
...
func (r *Runtime) removePod(ctx context.Context, p *Pod, removeCtrs, force bool) error {
	...
	if p.runtime.config.Libpod.CgroupManager == config.CgroupfsCgroupsManager {
	...
	if p.state.CgroupPath != "" {
		...
		switch p.runtime.config.Libpod.CgroupManager {
		case config.SystemdCgroupsManager:
		...
		default:
			...
			if removalErr == nil {
				removalErr = errors.Wrapf(define.ErrInternal, "unrecognized cgroup manager %s when removing pod %s cgroups", p.runtime.config.Libpod.CgroupManager, p.ID())
			} else {
				logrus.Errorf("Unknown cgroups manager %s specified - cannot remove pod %s cgroup", p.runtime.config.Libpod.CgroupManager, p.ID())
			}
		}
	}

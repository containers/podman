...
func defaultConfigFromMemory() (*Config, error) {
	...
	// We can't do an idiomatic struct init here since some values change for
	// rootless.
	c := new(Config)
	...
	c.StaticDir = filepath.Join(c.StorageConfig.GraphRoot, "libpod")
	c.VolumePath = filepath.Join(c.StorageConfig.GraphRoot, "volumes")
	...
	c.MaxLogSize = -1
	c.NoPivotRoot = false
	c.CNIConfigDir = etcDir + "/cni/net.d/"
	c.CNIPluginDir = []string{"/usr/libexec/cni", "/usr/lib/cni", "/usr/local/lib/cni", "/opt/cni/bin"}

...
func create(cmd *cobra.Command, args []string) error {
	...
	if len(args) > 0 {
		if len(createOptions.Name) > 0 {
			return errors.New("cannot specify --name and NAME at the same time")
		}
	...
	if !createOptions.Infra {
		if cmd.Flag("no-hosts").Changed {
			return errors.New("cannot specify --no-hosts without an infra container")
		}
		flags := cmd.Flags()
	...
	if len(createOptions.Net.PublishPorts) > 0 {
		if !createOptions.Infra {
			return errors.New("you must have an infra container to publish port bindings to the host")
		}
	}

...
### Quadlet
- Introduce Quadlet, a new systemd-generator that can be used to easily write and maintain systemd services using Podman.
- Quadlet now features a new option, `-dryrun`, which prints out parsed unit files to stdout instead of creating the files.
- Allow users to create podman networks using quadlet and link them to services created via .container and .kube files ([#16596](https://github.com/containers/podman/issues/16596)).
- Quadlet now supports running `podman kube play` via .kube files.

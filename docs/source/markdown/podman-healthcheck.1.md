...
Healthchecks are typically sourced from images or defined when creating a container via `podman run` or `podman create` using:
...
**Startup healthcheck** is a separate, one-time gate that runs *before* the regular healthcheck begins. It's designed for containers with extended or unpredictable startup times:
- Define it with `--health-startup-cmd` (requires a regular healthcheck to also be set)
- Once the startup healthcheck succeeds (based on `--health-startup-success`), it stops and the regular healthcheck takes over

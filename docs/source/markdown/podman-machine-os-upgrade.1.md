...
Only perform a dry-run of checking for the upgrade.  No content is downloaded or applied.

#### **--format**, **-f**

Define a structured output format.  The only valid value for this is `json`.
...
This is an advanced option only and is used internally by Podman.

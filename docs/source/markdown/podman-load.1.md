...
## DESCRIPTION
**podman load** loads an image from either an **oci-archive** or a **compat-archive** stored on the local machine into container storage. **podman load** reads from stdin by default or a file if the **input** option is set.
...
The local client further supports loading an **oci-dir** or a **dir** as created with **podman save** (1).

...
func prepareContainerFiles(containerFiles []string, contextDir string, options *BuildOptions, tempManager *TempFileManager) (*BuildFilePaths, error) {
	out := BuildFilePaths{
	...
	for _, c := range containerFiles {
		...
		if c == "/dev/stdin" {
			stdinFile, err := tempManager.CreateTempFileFromStdin("")
			if err != nil {
...
func prepareSecrets(secrets []string, contextDir string, tempManager *TempFileManager) ([]string, []string, error) {
	if len(secrets) == 0 {
		return nil, nil, nil
	}

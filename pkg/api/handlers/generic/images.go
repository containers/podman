...
func LoadImage(w http.ResponseWriter, r *http.Request) {
	...
	if len(r.Form.Get("quiet")) > 0 {
		...
		if err != nil {
			handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "bad value for quiet"))
			return
		}
	}
	f, err := ioutil.TempFile("", "api_load.tar")
	if err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "failed to create tempfile"))
		return
	}
	if err := saveFromBody(f, r); err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "failed to write temporary file"))
		return
	}
	id, err := runtime.LoadImage(r.Context(), "", f.Name(), writer, "")
	if err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "failed to load image"))
		return
	}
	_ = quiet
	handlers.WriteResponse(w, http.StatusOK, struct {
...
func ExportImage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		handlers.ImageNotFound(w, name, errors.Wrapf(err, "Failed to find image %s", name))
		return
	}
	tmpfile, err := ioutil.TempFile("", "api.tar")
	if err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "unable to create tempfile"))
		return
	}
	if err := tmpfile.Close(); err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "unable to close tempfile"))
		return
	}
	if err := newImage.Save(r.Context(), name, "docker-archive", tmpfile.Name(), []string{}, false, false); err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "failed to save image"))
		return
	}
	rdr, err := os.Open(tmpfile.Name())
	if err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "failed to read the exported tarfile"))
		return
	}
	handlers.WriteResponse(w, http.StatusOK, rdr)
}
...
func PruneImages(w http.ResponseWriter, r *http.Request) {
	...
	if len(r.Form.Get("filters")) > 0 {
		if err := json.Unmarshal([]byte(r.Form.Get("filters")), &filters); err != nil {
			handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "marshalling filters"))
			return
		}
	}
	// until ts is not supported on podman prune
	if len(filters["until"]) > 0 {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "until is not supported yet"))
		return
	}
	// labels are not supported on podman prune
	if len(filters["label"]) > 0 {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "labelis not supported yet"))
		return
	}
	...
	if len(filters["dangling"]) > 0 {
		...
		if err != nil {
			handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "processing dangling filter"))
			return
		}
	}
	...
	if err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "unable to get images to prune"))
		return
	}
	for _, p := range pruneImages {
		repotags, err := p.RepoTags()
		if err != nil {
			handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "unable to get repotags for image"))
			return
		}
		if err := p.Remove(r.Context(), true); err != nil {
			...
			handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "failed to prune image"))
			return
		}
	...
	handlers.WriteResponse(w, http.StatusOK, handlers.ImagesPruneReport{ImagesPruneReport: ipr})
}
...
func CommitContainer(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusNotFound, err)
		return
	}
	...
	if err != nil && !strings.Contains(err.Error(), "is not running") {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrapf(err, "CommitFailure"))
		return
	}
	handlers.WriteResponse(w, http.StatusOK, handlers.IDResponse{ID: commitImage.ID()}) // nolint
}
...
func CreateImageFromSrc(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrap(err, "unable to import tarball"))
		return
	}
...
func GetImage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		handlers.Error(w, "Server error", http.StatusInternalServerError, errors.Wrapf(err, "Failed to convert ImageData to ImageInspect '%s'", inspect.ID))
		return
	}
...
func GetImages(w http.ResponseWriter, r *http.Request) {
	...
	if t != "" {
		...
		if err != nil {
			handlers.Error(w, "Server error", http.StatusBadRequest, errors.Wrapf(err, "Failed to parse 'all' parameter %s", t))
			return
		}
	}
	...
	if t != "" {
		...
		if err != nil {
			handlers.Error(w, "Server error", http.StatusBadRequest, errors.Wrapf(err, "Failed to parse 'digests' parameter %s", t))
			return
		}
	}
	...
	if err != nil {
		handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrapf(err, "Failed to obtain the list of images from storage"))
		return
	}
	...
	for j, img := range images {
		...
		if err != nil {
			handlers.Error(w, "Something went wrong.", http.StatusInternalServerError, errors.Wrapf(err, "Failed to convert storage image '%s' to API image", img.ID()))
			return
		}
		summaries[j] = is
	}

	handlers.WriteResponse(w, http.StatusOK, summaries)
}

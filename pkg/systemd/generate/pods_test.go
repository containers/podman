...
func TestConvertNameArg(t *testing.T) {
        tests := []struct {
                input, expected []string
        }{
                {
                        []string{"podman", "pod", "create", "pod_name"},
                        []string{"podman", "pod", "create", "--name", "pod_name"},
                },
                {
                        []string{"podman", "pod", "create", "--name", "foo", "--exit-policy=stop"},
                        []string{"podman", "pod", "create", "--name", "foo", "--exit-policy=stop"},
                },
        }
        for _, test := range tests {
                assert.Equalf(t, test.expected, convertNameArg(test.input), "%v", test.input)
        }
}
...
WantedBy=default.target
`
	podGoodNewNoNameArgNoExitPolicy := `# pod-123abc.service
# autogenerated by Podman CI
...
TimeoutStopSec=70
ExecStartPre=/bin/rm -f %t/pod-123abc.pid %t/pod-123abc.pod-id
ExecStartPre=/usr/local/bin/podman pod create --infra-conmon-pidfile %t/pod-123abc.pid --pod-id-file %t/pod-123abc.pod-id --name foo --exit-policy=stop
ExecStart=/usr/local/bin/podman pod start --pod-id-file %t/pod-123abc.pod-id
ExecStop=/usr/local/bin/podman pod stop --ignore --pod-id-file %t/pod-123abc.pod-id -t 10
ExecStopPost=/usr/local/bin/podman pod rm --ignore -f --pod-id-file %t/pod-123abc.pod-id
PIDFile=%t/pod-123abc.pid
Type=forking
...
`
	...
	tests := []struct {
		...
		{"pod without --name and without --exit-policy",
			podInfo{
			...
			podGoodNewNoNameArgNoExitPolicy,
			true,
			false,
			false,
		},

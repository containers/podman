...
ExecStartPre=/bin/rm \
	-f %t/%n.ctr-id
ExecStartPre=/usr/bin/podman pull awesome-image:latest
ExecStart=/usr/bin/podman run \
	--cidfile=%t/%n.ctr-id \
	--cgroups=no-conmon \
	--rm \
	...
	tests := []struct {
		...
		{"good with --pull",
			containerInfo{
				...
				GraphRoot:         "/var/lib/containers/storage",
				RunRoot:           "/var/run/containers/storage",
				CreateCommand:     []string{"I'll get stripped", "create", "awesome-image:latest"},
				EnvVariable:       define.EnvVariable,
				ExecStartPrePull:  "/usr/bin/podman pull awesome-image:latest",
			},
			goodNewWithPull,
			true,
			false,

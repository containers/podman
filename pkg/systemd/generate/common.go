...
const headerTemplate = `{{{{if or (.Header) (ne .NoDefaultHeader true) -}}}}
{{{{if (ne .NoDefaultHeader true) -}}}}
# {{{{.ServiceName}}}}{{{{- if (eq .IdentifySpecifier true) }}}}@{{{{- end}}}}.service
# autogenerated by Podman {{{{.PodmanVersion}}}}
{{{{- if .TimeStamp}}}}
# {{{{.TimeStamp}}}}
{{{{- end}}}}
{{{{- end}}}}
{{{{- if .Header }}}}{{{{if (ne .NoDefaultHeader true)}}}}
{{{{end}}}}{{{{.Header}}}}{{{{- end}}}}

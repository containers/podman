...
// ContainerExecute is the varlink endpoint to execute a command in a container
func (i *LibpodAPI) ContainerExecute(call iopodman.VarlinkCall, opts iopodman.ExecOpts) error {
	...
	go func() {
		fmt.Printf("ContainerExec Start ctr.Exec\n")
		err := ctr.Exec(opts.Tty, opts.Privileged, envs, opts.Cmd, user, workDir, streams, 0)
		if err != nil {
			fmt.Printf("ContainerExec Exec err %s, %s\n", err.Error(), errors.Cause(err).Error())
			errChan <- errors.Wrapf(err, "ContainerExec failed for container %s", ctr.ID())
		}
	}()
	...
	if execErr != nil && errors.Cause(execErr) != io.EOF {
		fmt.Printf("ContainerExec err: %s\n", execErr.Error())
		return call.ReplyErrorOccurred(execErr.Error())
	}

	if err = virtwriter.HangUp(writer); err != nil {
		fmt.Printf("ContainerExec hangup err: %s\n", err.Error())
		logrus.Errorf("ContainerExec failed to HANG-UP on %s: %s", ctr.ID(), err.Error())
	}

...
func (c *CreateConfig) getContainerCreateOptions(runtime *libpod.Runtime, pod *libpod.Pod, mounts []spec.Mount, namedVolumes []*libpod.ContainerNamedVolume) ([]libpod.CtrCreateOption, error) {
	...
	logMap := getLoggingOpts(c.LogDriverOpt)

	logPath := logMap["path"]
	logTag := logMap["tag"]
	logSizeMax := logMap["logsizemax"]

	intlogsizemax, err := strconv.ParseInt(logSizeMax, 10, 64)
	...
	if logSizeMax != "" {
		options = append(options, libpod.WithMaxLogSize(intlogsizemax))
	}

...
// getLoggingOpts splits the path= and tag= and max-size= options provided to --log-opt.
func getLoggingOpts(opts []string) map[string]string {
	var logMap map[string]string
	for _, opt := range opts {
		arr := strings.SplitN(opt, "=", 2)
		if len(arr) == 2 {
			if strings.TrimSpace(arr[0]) == "path" {
				logMap["path"] = strings.TrimSpace(arr[1])
			}
			if strings.TrimSpace(arr[0]) == "tag" {
				logMap["tag"] = strings.TrimSpace(arr[1])
			}
			if strings.TrimSpace(arr[0]) == "max-size" {
				logMap["logsizemax"] = strings.TrimSpace(arr[1])
			}
		}
		if logMap["path"] != "" && logMap["tag"] != "" && logMap["logsizemax"] != "" {
			break
		}
	}
	return logMap

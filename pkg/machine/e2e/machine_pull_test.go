...
func pullOCITestDisk(finalDir string, vmType define.VMType) error {
	...
	ociArtPull, err := ocipull.NewOCIArtifactPull(context.Background(), &dirs, "docker://quay.io/podman/machine-os", "e2emachine", vmType, unusedFinalPath, skipTlsVerify, nil)
	if err != nil {
		return err
	}

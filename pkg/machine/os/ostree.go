...
func (dist *OSTree) Upgrade(opts UpgradeOptions) error {
	var (
	...
	switch compareMajorMinor(opts.MachineVersion, opts.ClientVersion) {
	case -1:
		// if caller version < machine version, return an error because this is bad
		return fmt.Errorf("client version %s is older than your machine version %s: upgrade your client", opts.ClientVersion, opts.MachineVersion)
	case 0:
		...
		if compareMajorMinor(*originVersion, opts.ClientVersion) != 0 {
			return fmt.Errorf("version mismatch between podman version and host os")
		}
		...
		registryDigest, updateImageDigest, updateExists, err := checkInBandUpgrade(originNamed, localDigest)
		if err != nil {
			return err
		}
		...
		updateReference = fmt.Sprintf("%s@%s", originNamed.Name(), updateImageDigest.String())
	default:
...
func checkInBandUpgrade(named reference.Named, localHostDigest digest.Digest) (digest.Digest, digest.Digest, bool, error) {
	...
	return registryImageManifest.ConfigInfo().Digest, d, registryImageManifest.ConfigInfo().Digest != localHostDigest, nil
}

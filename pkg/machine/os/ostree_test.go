...
func Test_compareMajorMinor(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "equal major and minor versions",
			args: args{
		...
		{
			name: "equal major and minor with different patch",
			args: args{
				versionA: semver.MustParse("2.5.10"),
		...
		{
			name: "patch difference should be ignored",
			args: args{
				versionA: semver.MustParse("5.7.999"),
				versionB: semver.MustParse("5.7.0"),
			},
			want: 0,
		},
		{
